...
public function kobe(){
    ...
    public function zero(){
        ...
        if(isset($_GET['xcode'])){
            $request_data = array();
            $tmp = explode('-', $_GET['xcode']);

            if(!($ori_id = $tmp[0])){
                exit('ori_id required');
            }

            if(!($cur_id = $tmp[1])){
                exit('cur_uid required');
            }
        }else{
            if(!$ori_id = $this->input->post('ori_id')){
                exit('ori_uid required');
            }

            if(!$cur_id = $this->input->post('cur_id')){
                exit('cur_id required');
            }
        }
        ...
        {
            //触发微信返回code码
            $params_format = "{$ori_id}-{$cur_id}";
            $suffix = "xcode={$params_format}";
            $url = $jsApi->createOauthUrlForCode(WxPayConf_pub::JS_API_CALL_URL . '?' . $suffix);
            Header("Location: $url");
        }else
        {
        ...
        //sign已填,商户无需重复填写
        $unifiedOrder->setParameter("openid","$openid");//商品描述
        $unifiedOrder->setParameter("body","超级Home 0元众筹装修");//商品描述
        //自定义订单号，此处仅作举例
        $timeStamp = time();
        ...
        $unifiedOrder->setParameter("notify_url", "http://www.90pingfang.com/wechat/pay/zero_notify");//通知地址
        $unifiedOrder->setParameter("trade_type","JSAPI");//交易类型
        //非必填参数，商户可根据实际情况选填
        //$unifiedOrder->setParameter("sub_mch_id","XXXX");//子商户号
        //$unifiedOrder->setParameter("device_info","XXXX");//设备号 
        $unifiedOrder->setParameter("attach","{$ori_id}-{$cur_id}");//附加数据
        //$unifiedOrder->setParameter("time_start","XXXX");//交易起始时间
        //$unifiedOrder->setParameter("time_expire","XXXX");//交易结束时间
        //$unifiedOrder->setParameter("goods_tag","XXXX");//商品标记
        //$unifiedOrder->setParameter("openid","XXXX");//用户标识
        ...
        $this->load->view('activity/zero/wepay', $data);
    }

    public function zero_notify(){
        //使用通用通知接口
        $notify = new Notify_pub();
        ...
        //以log文件形式记录回调信息
        $log_ = new Log_();
        $log_name= $this->config->item('log_path') . "zero_notify.log";//log文件路径
        $log_->log_result($log_name,"【接收到的notify通知】:\n".$xml."\n");
        ...
        if($notify->checkSign() == TRUE)
        {
            ...
            if($this->zero->update_money($ori_uid, $cur_uid)){
                $log_->log_result($log_name,"【update succ】:\n");
            }else{
                $log_->log_result($log_name,"【update fail】:\n");
            }
        }else{
            $log_->log_result($log_name,"checkSign fail\n");
        }
