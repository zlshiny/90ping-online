...
<script type="text/javascript">
    $(function(){
        $('#arrow-img').on('click',function(){
            var display = $('#my-order-content').css('display');
            if(display == 'none'){
                $('#my-order-content').show();
                $(this).attr('src','/static/image/mobile/arrow-up.png');
            }else{
                $('#my-order-content').hide();
    ...
    <div class="order-detail">
        ...
        <?php foreach($list as $v):?>
            <div class="order-menu">
                ...
                <div class="arrow">
                    <img id="arrow-img" src="/static/image/mobile/arrow-up.png">
                </div>
            </div>
            <div class="my-order-content" id="my-order-content">
                ...
                <div class="my-order-operation">
                    ...
                    <?php if($v['status'] == 0):?>
                        <a href="#" class="my-order-a" id="action_order" serial-number="<?=$v['serial_number'];?>" order-id="<?=$v['order_id'];?>" user-id="<?=$v['user_id'];?>" value="<?=$v['status'];?>">
                            <?=$v['action_name'];?>
                        </a>
                    <?php endif;?>
                    <div></div>
